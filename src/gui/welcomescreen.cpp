	...
	: QFrame(parent), m_collection(collection)
{
	...
	KConfigGroup gameGroup (KSharedConfig::openConfig(), QStringLiteral("KSudokuGame"));
...
void WelcomeScreen::startEmptyGame() {
	GameVariant* variant = selectedVariant();
	if(!variant) {
		KMessageBox::error(this, i18n("Please select a puzzle variant."), i18nc("@title:window", "Unable to Start Puzzle"));
		return;
	}
	
	Game game = variant->startEmpty();
	if (! game.isValid()) {
		KMessageBox::error(this, i18n("Unable to create an empty puzzle of the chosen variant; please try another."), i18nc("@title:window", "Unable to Start Puzzle"));
		return;
	}
...
void WelcomeScreen::playVariant() {
	...
	if(!game.isValid()) {
		KMessageBox::error(this, i18n("Unable to start a puzzle of the chosen variant; please try another."), i18nc("@title:window", "Unable to Start Puzzle"));
		return;
	}
...
void WelcomeScreen::generatePuzzle() {
	...
	if(!game.isValid()) {
		KMessageBox::error(this, i18n("Unable to generate a puzzle of the chosen variant; please try another."), i18nc("@title:window", "Unable to Start Puzzle"));
		return;
	}
